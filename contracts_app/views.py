...
def export_contracts_excel(request):
    ...
    if request.GET:
        if q := request.GET.get('q'):
            contracts = contracts.filter(Q(customer_name__icontains=q) | Q(customer_inn__icontains=q))
        if ak_number := request.GET.get('ak_number'):
            contracts = contracts.filter(aks__number=ak_number)
        if ak_district := request.GET.get('ak_district'):
            contracts = contracts.filter(aks__district__name__icontains=ak_district.strip())
        ...
        if works := request.GET.getlist('works'):
            contracts = contracts.filter(works__pk__in=works)
        checklist_filters = Q()
        if request.GET.get('gos_services'): checklist_filters |= Q(gos_services=True)
        if request.GET.get('oko'): checklist_filters |= Q(oko=True)
        if request.GET.get('spolokh'): checklist_filters |= Q(spolokh=True)
        if checklist_filters: contracts = contracts.filter(checklist_filters)
    ...
    headers = [
        "№", "Заказчик", "ИНН", "Исполнитель", "Срок действия",
        "Статус", "Кол-во АК", "АК (номера)", "Работы", "Сумма общая", "В мес",
        "Госуслуги", "ОКО", "Сполох"
    ]
    ws.append(headers)
    ...
    for idx, contract in enumerate(contracts, start=1):
        ...
        ak_numbers = " | ".join(str(ak.number) for ak in contract.aks.all()) if contract.aks.exists() else ""
        works_list = " | ".join(w.name for w in contract.works.all()) if contract.works.exists() else ""
